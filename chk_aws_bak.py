...
img_chunk = 3500 #images

for i in range(len(image_paths)/img_chunk+1):
    features_indices = (i*img_chunk,i*img_chunk+len(image_paths%img_chunk)-1 if (i+1)*img_chunk>(len(image_paths)) else (i+1)*img_chunk-1)
...
#prep first image
    ...
    for idx,image_path in enumerate(image_paths[1:element_num]):
    ...
    if image_data.shape[0]<element_num:
        np.append(image_data,image_data_temp,axis = 0)
    #ipdb.set_trace()
